...
applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact)
{
	...
	*h = MAX(1, *h);
	...
	if (*w < bh)
		*w = bh;
	if (resizehints || c->isfloating || !layouts[c->mon->sellt].arrange) {
...
configurerequest(XEvent *e)
{
	...
	if ((c = wintoclient(ev->window))) {
		...
		else if (c->isfloating || layouts[selmon->sellt].arrange) {
			m = c->mon;
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			if (m->click_kills)
				drw_setscheme(drw, scheme[SchemeKill]);
			else
...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		resize(c, m->wx, m->wy, m->ww - 2 * c->bw, m->wh - 2 * c->bw - dockh, 0);
}
...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MotionNotify:
			...
			if (!c->isfloating && layouts[selmon->sellt].arrange
			&& (abs(nx - c->x) > snap || abs(ny - c->y) > snap))
				togglefloating(NULL);
			if (!layouts[selmon->sellt].arrange || c->isfloating)
...
restack(Monitor *m)
{
	...
	if (!m->sel)
		return;
	if (m->sel->isfloating || !layouts[m->sellt].arrange)
...
{
	selmon->sellt = arg->i == -1 ? (selmon->sellt + 1) % LENGTH(layouts) : arg->i;
	strncpy(selmon->ltsymbol, layouts[selmon->sellt].symbol, sizeof selmon->ltsymbol);
	if (selmon->sel)
		arrange(selmon);
	else
		drawbar(selmon);
...
setmfact(const Arg *arg)
{
	...
	if (!arg || !layouts[selmon->sellt].arrange)
		return;
...
showhide(Client *c)
{
	...
	if (ISVISIBLE(c)) {
		...
		if ((!layouts[c->mon->sellt].arrange || c->isfloating) && !c->isfullscreen)
...
tile(Monitor *m)
{
	...
	for (i = 0, my = ty = m->gap->gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + m->gap->gappx, m->wy + my, mw - (2*c->bw) - m->gap->gappx, h - (2*c->bw)  - dh, 0);
			if (my + HEIGHT(c) + m->gap->gappx < m->wh)
				my += HEIGHT(c) + m->gap->gappx;
		} else {
			h = (m->wh - ty) / (n - i) - m->gap->gappx;
			resize(c, m->wx + mw + m->gap->gappx, m->wy + ty, m->ww - mw - (2*c->bw) - 2*m->gap->gappx, h - (2*c->bw) - dh, 0);
...
zoom(const Arg *arg)
{
	...
	if (!layouts[selmon->sellt].arrange
